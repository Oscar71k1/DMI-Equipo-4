...
La revisión inicial conservó los hashes de workflows, evaluador y pruebas públicas. Posteriormente Oscar autorizó añadir `tests/security.test.ts` al comando existente de regresiones y cerrar con `week-03-final`. Ese es el único cambio adicional del YAML; evaluador y pruebas públicas permanecen intactos. La ejecución de la etiqueta debe aprobar también AC-03 y conservar los artefactos.
