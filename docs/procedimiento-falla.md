...
En Windows, el evaluador público no encontraba el comando `npm` desde Python. Durante la preparación se ajustó su lanzador para resolver `npm.cmd` y la codificación de salida. Tras la aclaración de conservar íntegro el material de evaluación, se retiraron esos ajustes y se restauró `tools/course_public_evaluator.py` desde el commit inicial `635d471c3bce751720adbe0e2c50bcd245520d51`. En esa etapa, las pruebas de `course-tests/`, `package.json` y `Makefile` se conservaban originales. En `.github/workflows/week-01-feedback.yml` se añadió entonces `fetch-depth: 0` para consultar el padre del commit de evidencias. El diagnóstico histórico de ese ajuste está en [diagnostico-github-actions.md](diagnostico-github-actions.md). Posteriormente se ensayó la preparación del historial desde `make setup`. Ese ajuste se retiró: la versión vigente conserva el Makefile original y configura el checkout de Semana 1 con `ref` explícito y `fetch-depth: 2`, como se explica en [preparacion-historial-git.md](preparacion-historial-git.md). Las pruebas y el evaluador permanecen originales.
