...
En Windows, el evaluador público no encontraba el comando `npm` desde Python. Durante la preparación se ajustó su lanzador para resolver `npm.cmd` y la codificación de salida. Tras la aclaración de conservar íntegro el material de evaluación, se retiraron esos ajustes y se restauró `tools/course_public_evaluator.py` desde el commit inicial `635d471c3bce751720adbe0e2c50bcd245520d51`. Las pruebas de `course-tests/`, los workflows, `package.json` y `Makefile` nunca se modificaron.

Los resultados obtenidos con el evaluador modificado son antecedentes, no una validación final del evaluador original. La compatibilidad del entorno de Windows sigue pendiente hasta ejecutar las comprobaciones originales correctamente. La evidencia de la falla y su corrección procede directamente de `npm run test:smoke` con la prueba original y no depende de los cambios que se retiraron del evaluador.
